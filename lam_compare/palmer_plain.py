...
lam_3d = lam_3d.assign(
    ...
    * (
        mtf.relative_humidity_to_specific_humidity(
            RH=1, p=lam_3d.pfull, T=lam_3d.ta, es=svp.liq_wagner_pruss
        )
        - lam_3d.qv
    ),
)
...
delta_mse = (
    lam_3d.mse.sel(z=1500, method="nearest") - lam_3d.mse.sel(z=6000, method="nearest")
...
mse_sat_hist = histogram(delta_mse, delta_q, bins=[bins_mse, bins_q])
...
beach = beach.assign(
    ...
    * (
        mtf.relative_humidity_to_specific_humidity(
            RH=1, p=beach.p, T=beach.ta, es=svp.liq_hardy
        )
        - beach.q
    ),
)
...
beach = beach.assign(
    delta_mse=(
        beach.mse.sel(altitude=1500, method="nearest")
        - beach.mse.sel(altitude=6000, method="nearest")
    )
    * 1e-3,
