...
def get_hist_of_ta_2d(da_t, da_var, var_binrange, ta_binrange=(240, 305), var_nb=100, ta_nb=200):
...
ifs = xr.open_dataset("/work/mh0492/m301067/orcestra/results/timeseries/ifs_interpolated_on_dropsondes_profiles_2nd-days.nc")

cids = get_cids()
beach =open_dropsondes(cids["dropsondes"]).pipe(interpolate_gaps).pipe(extrapolate_sfc).isel(sonde=slice(2, None))

gate = (
            xr.open_dataset("ipfs://QmWZryTDTZu68MBzoRDQRcUJzKdCrP2C4VZfZw1sZWMJJc", engine="zarr")
        ...
        ).pipe(interpolate_gaps).pipe(extrapolate_sfc)
#%% create lam dataset
...
lam_subset = lam_ds.sortby(altdim, ascending=False).reset_coords(["launch_lat", "launch_lon"])
...
ifs = ifs.assign(
    n2 = help.apply_brunt_vaisala_frequency(ifs, altdim="altitude", q="q")
)
...
n2_pseudo = mtf.brunt_vaisala_frequency(pseudo2["theta"], pseudo2["q"], pseudo2["altitude"])
...
ice_histograms = {}
n2_hist = {}
for ds, name in [(lam_subset, "lam-total"), 
                  (ifs, "ifs-total"),
                   (beach, "beach-total"), 
...
kwargs = dict(
    lam={
        "contour":dict(cmap="Oranges", add_colorbar=False),#sns.light_palette("darkorange",n_colors=7, as_cmap=True)),
...
for name, label in [(f"lam-{region}", "ICON"), (f"beach-{region}", "Dropsondes")]:
    plthist = xr.concat(
       ...
       ((ice_histograms[name] / ice_histograms[name].sum("ice_rh_bin"))
          ...
          .rolling(ta_bin=20, center=True, min_periods=3).sum()
          .rolling(ice_rh_bin=7, center=True, min_periods=3).sum()
        ...
        ((rh_histograms[name] / rh_histograms[name].sum("rh_bin"))
          ...
          .rolling(ta_bin=15, center=True, min_periods=3).sum()
          .rolling(rh_bin=7, center=True, min_periods=3).sum()
       ),
...
ax.text( 1.05, 250,
        "RH wrt ice", rotation=90,
        va="center",
        color="black", fontsize=16)
ax.text( 1.02, 290,
        "RH wrt \nliquid water", rotation=90,
        va="center",
        color="black", fontsize=16)
...
region="total"
for name, label in [(f"lam-{region}", "ICON"), (f"beach-{region}", "Dropsondes")]:
    plthist = n2_hist[name].rolling(ta_bin=5, center=True, min_periods=3).sum().where(n2_hist[name]>0)
