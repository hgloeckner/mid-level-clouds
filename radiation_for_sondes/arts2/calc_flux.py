...
def get_encoding(dataset):
    ...
    return {
        ...
        for var in dataset.variables if var != "sonde_id"
    }
...
def init_calc(ds):
    ...
    f_grid = np.linspace(min_wavenumber, max_wavenumber, wave_bands)  # frequency grid in cm^-1
...
def create_ds(ds, f_grid):
    ...
    fluxes =  xr.Dataset(
        {
            "lw_flux_up_spectral": (("sonde", "altitude", "f_grid"), np.full(shape_flux, np.nan)),
            "lw_flux_down_spectral": (("sonde", "altitude", "f_grid"), np.full(shape_flux, np.nan)),
        ...
        coords={"sonde_id": ("sonde",ds.sonde_id.values), "altitude": ("altitude", ds.altitude.values), "f_grid": ("f_grid", f_grid)}
    )
    return xr.merge(
        [fluxes.reset_coords(['sonde_id']), ds], compat="override"
    )
...
def initialize_store(store, ds):
    ...
    lw_fluxes[["sonde_id","altitude", "f_grid", 'N2', 'mag_w', 'mag_u', 'O3', 'CO2', 'wind_v', 'wind_w', 'wind_u', 'O2', 'mag_v']].to_zarr(
        store,
        mode="r+",
    )
...
def calc_fluxes(ds):
    ...
    for i in range(ds.sonde.size):
        ...
        print(f"{i + 1}/{ds.sonde.size} complete | ETA: {remaining:.1f}s (~{remaining / 60:.1f} min)", flush=True)
    return lw_fluxes
...
def write_region(store, region):
    ...
    lw_fluxes.drop_vars(["sonde_id","altitude", "f_grid",'N2', 'mag_w', 'mag_u', 'O3', 'CO2', 'wind_v', 'wind_w', 'wind_u', 'O2', 'mag_v']).to_zarr(
        store,
        mode="r+",
        region=region,
    )
