...
#%%
era5 = xr.open_dataset(
    "/scratch/m/m301046/radiation_profiles.nc"
)
#%%          
...
def merge_ds(sonde, era5):
    ...
    ).chunk({"altitude":-1}).interpolate_na(dim="altitude").assign_coords({"altitude":new_sonde.altitude})

merged_profiles =[]
for sondeid in beach.sonde:
    print(sondeid.values)
    sonde = beach.isel(sonde=sondeid)
    era5_sonde = era5.sel(sonde=sondeid)#(valid_time=sonde.launch_time,latitude=sonde.launch_lat, longitude=sonde.launch_lon, method="nearest")
...
#%%
ds = xr.concat(merged_profiles, dim="sonde").assign(
    sonde_id = sids
)

ds = ds.dropna(how="any", dim="sonde", subset=["ta", "q", "p"])


#%%
ds.to_netcdf("/work/mh0066/m301046/data/mlclouds/radiation_profiles.nc")
