...
def find_highest_cloud_altitude(
    ds,  variable_name="bsrgl", threshold=20
):
...
cloud_top = xr.DataArray(data=find_highest_cloud_altitude(no_wv), dims="sonde", name="cloud_top").to_dataframe()
#%%

east = [[-34.5, 2.5], [-34.5, 13.5], [-20, 13.5], [-20, 2.5]]
west = [[-59, 6], [-59, 17], [-44.5, 17], [-44.5, 6]]
north = [[-26, 13.5], [-26, 19], [-20, 19], [-20, 13.5]]
cloud_top_north = xr.DataArray(data=find_highest_cloud_altitude(sel_sub_domain(no_wv, north,item_var="time", lon_var="longitude", lat_var="latitude")), dims="sonde", name="cloud_top").to_dataframe()
cloud_top_east = xr.DataArray(data=find_highest_cloud_altitude(sel_sub_domain(no_wv, east,item_var="time", lon_var="longitude", lat_var="latitude")), dims="sonde", name="cloud_top").to_dataframe()
cloud_top_west = xr.DataArray(data=find_highest_cloud_altitude(sel_sub_domain(no_wv, west,item_var="time", lon_var="longitude", lat_var="latitude")), dims="sonde", name="cloud_top").to_dataframe()
#%%
...
for region, height in zip(
    ...
    ds = sel_sub_domain(dropsondes, eval(region),item_var="sonde", lon_var="launch_lon", lat_var="launch_lat")
    print("ct temperature for", region, ": ", ds.ta.sel(altitude=height, method="nearest").mean("sonde").values, "K")