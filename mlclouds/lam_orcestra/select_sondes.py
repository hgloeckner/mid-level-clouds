...
lam_3d=xr.open_mfdataset(path_to_files, engine="zarr", parallel=True, chunks={"time":12, "height_full":-1, "cell":16384}).pipe(egh.attach_coords)
#%%
...
rapsodi = od.open_radiosondes(cids["radiosondes"]).pipe(dh.interpolate_gaps).pipe(dh.extrapolate_sfc)
#%%
import numpy as np
import healpix
...
def get_lat_lon_mask(dx, lat, lon):
    ...
    return healpix.ang2pix(egh.get_nside(dx), lon%360, lat, nest=egh.get_nest(dx),lonlat=True)


def sel_sonde(dx, sonde):
    icell = get_lat_lon_mask(dx, sonde.launch_lat.values % 180, sonde.launch_lon.values % 360)
    return dx.sel(cell=int(icell)).sel(time=sonde.launch_time.values, method="nearest")
lam_profiles = []
for sonde in beach.sonde:
    lam_profiles.append(sel_sonde(lam_3d, beach.isel(sonde=sonde)).assign(
        sonde_id=beach.sonde_id.isel(sonde=sonde)
    ))
for sonde in rapsodi.sonde:
    lam_profiles.append(sel_sonde(lam_3d, rapsodi.isel(sonde=sonde)).assign(
        sonde_id=rapsodi.set_coords(["launch_lat", "launch_lon"]).sonde_id.isel(sonde=sonde)
    ))
...
xh.write_ds(lam_complete.reset_coords(
    ["height_full"]
).chunk({"sonde": -1, "z": -1}), "/scratch/m/m301046/", "lam_sondes_z.zarr", object_dims=("sonde",), alt_dim="z")
...
#%%
imerg_sondes.chunk({"sonde_id":-1}).to_zarr("/scratch/m/m301046/imerg_sondes.zarr", mode="w")

#%%
imerg_area = ds.sel(lat = slice(-8, 27.5), lon=slice(-68,-2)).sel(time=lam_3d.time, method="nearest")
#%%
imerg_area.chunk({"time":200, "lat":200, "lon":600}).to_zarr("/scratch/m/m301046/imerg_lam.zarr", mode="w")
#%%


ifs_sondes = xr.open_dataset("/work/mh0492/m301067/orcestra/results/timeseries/ifs_interpolated_on_dropsondes_profiles_2nd-days.nc")
...
#%%
lam_2d_sondes.isel(time=0).to_zarr("/scratch/m/m301046/lam_sondes_2d.zarr", mode="w", zarr_format=2)
